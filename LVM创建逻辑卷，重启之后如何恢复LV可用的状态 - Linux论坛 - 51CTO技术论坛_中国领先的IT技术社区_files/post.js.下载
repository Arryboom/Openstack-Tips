...
function selectfid(fid){
    $('fid').value=fid;
    $('forum-name-sel').innerHTML=$('forumname_'+fid).innerHTML;
    $('typelistdiv').style.display = 'none';
    //ajaxget('ajax.php?action=readthreadtype&fid='+fid,'typeoftr');
    //showdiv('typeoftr');
    var x=new Ajax('HTML');
    x.get('ajax.php?action=readthreadtype&fid='+fid+'&rand='+Math.random(),function(s){
    	$('typeoftr').innerHTML = s;
    });
}
jQuery(function(){
	jQuery('#forum-name-sel').parent().hover(
		function(){
			jQuery('#forum-name-sel').next().show();
		},function(){
			jQuery('#forum-name-sel').next().hide();	
		}
	)
})
function showdiv(id){
